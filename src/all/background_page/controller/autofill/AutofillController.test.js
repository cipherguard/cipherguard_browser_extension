...
describe("AutofillController", () => {
  ...
  describe("AutofillController::exec", () => {
    it("Should autofill from inform menu.", async() => {
      ...
      expect(portWrapper.emit).toHaveBeenCalledWith('cipherguard.web-integration.fill-credentials', {username: resource.metadata.username, password: secret.password});
      expect(portWrapper.emit).toHaveBeenCalledWith('cipherguard.in-form-menu.close');
    });
    ...
    it("Should autofill from quickaccess.", async() => {
      ...
      expect(portWrapper.request).toHaveBeenCalledWith('cipherguard.quickaccess.fill-form', resource.metadata.username, secret.password, tab.url);
      expect(portWrapper.emit).not.toHaveBeenCalledWith('cipherguard.in-form-menu.close');
    });
    ...
    it("Should not autofill from a worker that is not inform menu or quickaccess.", async() => {
      ...
      expect(controller.resourceTypeModel.getSecretSchemaById).toHaveBeenCalledTimes(1);
      expect(controller.resourceTypeModel.getSecretSchemaById).toHaveBeenCalledWith(resource.resourceTypeId);
      expect(portWrapper.emit).not.toHaveBeenCalledWith('cipherguard.web-integration.fill-credentials', {username: resource.username, password: secret.password});
      expect(portWrapper.request).not.toHaveBeenCalledWith('cipherguard.quickaccess.fill-form', resource.username, secret.password, tab.url);
      expect(portWrapper.emit).not.toHaveBeenCalledWith('cipherguard.in-form-menu.close');
    });
    ...
    it("Should map username with empty string if not exist.", async() => {
      ...
      expect(controller.resourceTypeModel.getSecretSchemaById).toHaveBeenCalledTimes(1);
      expect(controller.resourceTypeModel.getSecretSchemaById).toHaveBeenCalledWith(resource.resourceTypeId);
      expect(portWrapper.emit).not.toHaveBeenCalledWith('cipherguard.web-integration.fill-credentials', {username: "", password: secret.password});
      expect(portWrapper.request).not.toHaveBeenCalledWith('cipherguard.quickaccess.fill-form', "", secret.password, tab.url);
      expect(portWrapper.emit).not.toHaveBeenCalledWith('cipherguard.in-form-menu.close');
    });
  });
});

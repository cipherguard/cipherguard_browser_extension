...
class AuthVerifyServerKeyController {
  ...
  async onVerifyError(error) {
    ...
    if (error.message && error.message.indexOf('no user associated') !== -1) {
       ...
       */
      (await WorkerService.get('AuthBootstrap', this.worker.tab.id)).port.emit('cipherguard.auth-bootstrap.remove-iframe');
    } else {
      try {
        if (!await this.canParseServerKey()) {
          // @deprecated with v3.6.0. Fix an issue users encounter while using an armored server key with multiple keys https://github.com/cipherguard/cipherguard_browser_extension/issues/150

...
class OnExtensionInstalledController {
  ...
  static async onBrowserUpdate() {
    ...
    /*
     * Reload only tabs that match cipherguard app url. Reload is necessary as the application loaded in the tab
     ...
     */
    await browser.tabs.query({}).then(reloadTabsMatchCipherguardAppUrl);
  }

  /**
   * Updates the Cipherguard icon in the toolbar according to the sign-in status of the current user.
   * @returns {Promise<void>}
   */
...
const closeTabWebStore = tabs => {
  const urlWebStoreRegex = `https:\/\/(chromewebstore.google.com|addons.mozilla.org)\/(.*)\/cipherguard`;
 ...
 * @param tabs
 */
const reloadTabsMatchCipherguardAppUrl = async tabs => {

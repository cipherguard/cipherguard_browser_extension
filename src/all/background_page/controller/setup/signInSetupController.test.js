...
describe("SignInSetupController", () => {
  describe("SignInSetupController::exec", () => {
    ...
    it("Should ask for SSO kits generation.", async() => {
      ...
      organizationSettings.cipherguard.plugins.sso = {
        enabled: true
      };
      ...
      await MockExtension.withConfiguredAccount();
      const account = new AccountEntity(defaultAccountDto());
      jest.spyOn(AccountTemporarySessionStorageService, "get").mockImplementationOnce(() => ({account: account, passphrase: "ada@cipherguard.com"}));
      ...
      expect.assertions(7);
      await controller.exec(true);
      expect(controller.authVerifyLoginChallengeService.verifyAndValidateLoginChallenge).toHaveBeenCalledWith(account.userKeyFingerprint, account.userPrivateArmoredKey, "ada@cipherguard.com");
      expect(PassphraseStorageService.set).toHaveBeenCalledWith("ada@cipherguard.com", -1);
      expect(PostLoginService.exec).toHaveBeenCalled();
      expect(GenerateSsoKitService.generate).toHaveBeenCalledWith("ada@cipherguard.com", "azure");
    ...
    it("Should remember the passphrase during 1 minutes if not requested by the user.", async() => {
      ...
      expect(PassphraseStorageService.set).toHaveBeenCalledWith("ada@cipherguard.com", 60);
    });
  });
});

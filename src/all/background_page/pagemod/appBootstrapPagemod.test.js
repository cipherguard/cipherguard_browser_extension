...
describe("AppBootstrap", () => {
  ...
  describe("AppBootstrap::isConstraintValidated", () => {
    it("Should be able to attach app bootstrap pagemod to browser frame", async() => {
      ...
      const result = await AppBootstrap.canBeAttachedTo({frameId: Pagemod.TOP_FRAME_ID, url: "https://cipherguard.dev/app"});
      expect(result).toBeTruthy();
    });

    each([
      {scenario: "No domain", url: "app", frameId: Pagemod.TOP_FRAME_ID},
      {scenario: "Not top frame", url: "https://cipherguard.dev/app", frameId: 1},
    ...
    it("Should not be able to attach a pagemod if the user is not authenticated", async() => {
      ...
      jest.spyOn(UserSettings.prototype, "getDomain").mockImplementation(() => "https://cipherguard");
      // process

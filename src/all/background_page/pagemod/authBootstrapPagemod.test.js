...
describe("AuthBootstrap", () => {
  ...
  describe("AuthBootstrap::canBeAttachedTo", () => {
    it("Should be able to attach auth bootstrap pagemod to browser frame", async() => {
      ...
      const result = await AuthBootstrap.canBeAttachedTo({frameId: Pagemod.TOP_FRAME_ID, url: "https://cipherguard.dev/auth/login"});
      expect(result).toBeTruthy();
    });

    each([
      {scenario: "No domain", url: "auth/login", frameId: Pagemod.TOP_FRAME_ID},
      {scenario: "Not top frame", url: "https://cipherguard.dev/auth/login", frameId: 1},

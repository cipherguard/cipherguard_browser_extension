...
export default class GetPassphraseService {
  ...
  async requestPassphrase(worker) {
    const requestResult = await worker.port.request('cipherguard.passphrase.request');
  ...
  async requestPassphraseFromQuickAccess() {
     ...
     * open and it will have to reply to the request "cipherguard.passphrase.request".
     */

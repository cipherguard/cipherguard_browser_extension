...
describe("GetPassphraseService", () => {
  ...
  const getTestWorker = () => {
    ...
    worker.port.addRequestListener("cipherguard.passphrase.request", async() => passphraseRequestResponse);
  ...
  describe("GetPassphraseService::getPassphrase", () => {
    ...
    it("should return the user's private key's passphrase stored in the local storage", async() => {
      ...
      worker.port.addRequestListener("cipherguard.passphrase.request", () => { throw new Error("Should not be called during this test"); });
      const service = new GetPassphraseService(account);
      const receivedPassphrase = await service.getPassphrase(worker);
  ...
  describe("GetPassphraseService::requestPassphrase", () => {
    it("should call `cipherguard.passphrase.request` to retrieve passphrase from a user input", async() => {
      expect.assertions(1);
      const worker = getTestWorker();
      const service = new GetPassphraseService(account);
    ...
    it("should throw an exception if the user doesn't input its passphrase", async() => {
      ...
      const worker = getTestWorker();
      worker.port.addRequestListener("cipherguard.passphrase.request", () => { throw expectedError; });
      const service = new GetPassphraseService(account);
      try {

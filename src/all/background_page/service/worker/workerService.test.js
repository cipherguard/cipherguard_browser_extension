...
describe("WorkerService", () => {
  ...
  describe("WorkerService::get", () => {
    it("Should get worker", async() => {
      ...
      // expectations
      expect(BrowserTabService.sendMessage).toHaveBeenCalledWith(worker, "cipherguard.port.connect", worker.id);
  ...
  describe("WorkerService::destroyWorkersByName", () => {
    it("Destroy workers", async() => {
      ...
      // expectation
      expect(portWrapper.emit).toHaveBeenCalledWith("cipherguard.content-script.destroy");
      expect(portWrapper.emit).toHaveBeenCalledTimes(1);
      expect(portWrapper2.emit).toHaveBeenCalledWith("cipherguard.content-script.destroy");

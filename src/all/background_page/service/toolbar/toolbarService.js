...
class ToolbarService {
  ...
  handleShortcutPressed(command) {
    if (command === "cipherguard-open") {
      this.openCipherguardTab();
    }
  }
  ...
  /**
   * Open a new tab and go to cipherguard.
   */
  openCipherguardTab() {
  ...
  async updateSuggestedResourcesBadge() {
    try {
      ...
      if (!this.isUrlCipherguardDomain(this.tabUrl, account) && !this.isUrlCipherguardExtension(this.tabUrl)) {
   ...
   * @returns {boolean}
   */
  isUrlCipherguardDomain(tabUrl, account) {
    const cipherguardDomain = new URL(account.domain);
    const url = tabUrl && new URL(tabUrl);
    return cipherguardDomain.hostname === url?.hostname;
  }

  /**
   * Check if the url is a cipherguard extension url.
   * @param {string} tabUrl The url to check.
   * @returns {boolean}
   */
  isUrlCipherguardExtension(tabUrl) {

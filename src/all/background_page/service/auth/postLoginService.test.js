...
describe("PostLoginService", () => {
  describe("PostLoinService::postLogin", () => {
    it("Should send message to awake port and send post logout event", async() => {
      ...
      expect(BrowserTabService.sendMessage).toHaveBeenCalledWith(workerCta, "cipherguard.port.connect", workerCta.id);
      expect(informCtaPortWrapper.emit).toHaveBeenCalledWith('cipherguard.auth.after-login');

...
import IsEmailValidator from "cipherguard-styleguide/src/shared/lib/Validator/IsEmailValidator";
import {
...
import IsRegexValidator from "cipherguard-styleguide/src/shared/lib/Validator/IsRegexValidator";
...
describe("AppEmailValidatorService", () => {
  ...
  describe("AppEmailValidatorService::validate", () => {
    it("should validate standard email if application settings did not customize the email regex validation.", async() => {
      expect.assertions(2);
      expect(AppEmailValidatorService.validate("ada@cipherguard.com")).toBeTruthy();
      expect(AppEmailValidatorService.validate("ada@cipherguard.c")).toBeFalsy();
    });
    ...
    it("should validate custom email if the application settings customize the email regex validation.", async() => {
      ...
      const organizationSettings = customEmailValidationProOrganizationSettings();
      OrganizationSettingsModel.set(new OrganizationSettingsEntity(organizationSettings));
      expect(AppEmailValidatorService.validate("ada@cipherguard.c")).toBeTruthy();
      expect(AppEmailValidatorService.validate("ada@cipherguard.lu")).toBeFalsy();
    });
  });
});

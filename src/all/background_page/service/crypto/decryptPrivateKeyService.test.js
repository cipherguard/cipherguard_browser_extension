...
describe("DecryptPrivateKey service", () => {
  it('should validate a private key with the right passphrase', async() => {
    ...
    await expect(DecryptPrivateKeyService.decrypt(key, "ada@cipherguard.com")).resolves.not.toBeNull();
  }, 10 * 1000);
  ...
  it('should validate a private key armored and decrypt it', async() => {
    expect.assertions(1);
    await expect(DecryptPrivateKeyService.decryptArmoredKey(pgpKeys.ada.private, "ada@cipherguard.com")).resolves.not.toBeNull();
  }, 10 * 1000);
});

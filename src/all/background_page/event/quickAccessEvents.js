...
const listen = function(worker, apiClientOptions, account) {
   ...
   *
   * @listens cipherguard.quickaccess.use-resource-on-current-tab
   * @param requestId {uuid} The request identifier
   * @param resourceId {uuid} The resource identifier
   */
  worker.port.on('cipherguard.quickaccess.use-resource-on-current-tab', async(requestId, resourceId, tabId) => {
    let tab;
    if (!worker.port) {
   ...
   *
   * @listens cipherguard.resources.prepare-create
   * @param requestId {uuid} The request identifier
   * @param tabId {string} The tab id
   */
  worker.port.on('cipherguard.quickaccess.prepare-resource', async(requestId, tabId) => {
    try {
   ...
   *
   * @listens cipherguard.resources.prepare-autosave
   * @param requestId {uuid} The request identifier
   */
  worker.port.on('cipherguard.quickaccess.prepare-autosave', async requestId => {
    try {
      const resourceInProgress = await ResourceInProgressCacheService.consume() || {};
      worker.port.emit(requestId, 'SUCCESS', resourceInProgress);
    } catch (error) {
   ...
   *
   * @listens cipherguard.quickaccess.update-window-height
   * @param height {int} the height to apply
   */
  worker.port.on('cipherguard.quickaccess.update-window-height', async height => {
    try {
   ...
   */
  worker.port.on('cipherguard.users.find-logged-in-user', async(requestId, refreshCache = false) => {

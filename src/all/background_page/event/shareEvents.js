...
const listen = function(worker, apiClientOptions, account) {
   ...
   * @listens cipherguard.share.get-folders
   * @param {array} foldersIds The ids of the folders to retrieve.
   */
  worker.port.on('cipherguard.share.get-folders', async(requestId, foldersIds) => {
    try {
   ...
   * @listens cipherguard.share.submit
   * @param requestId {uuid} The request identifier
   * @param resourcesId {array<string>} The resources ids to share
   * @param permissionChangesDto {array} The permission changes
   */
  worker.port.on('cipherguard.share.resources.save', async(requestId, resourcesIds, permissionChangesDto) => {
   ...
   *
   * @listens cipherguard.share.folders.save
   * @param requestId {uuid} The request identifier
   * @param folderId {uuid} The folder id to share
   * @param permissionChangesDto {array} The permission changes
   */
  worker.port.on('cipherguard.share.folders.save', async(requestId, folderId, permissionChangesDto) => {

...
const listen = function(worker, apiClientOptions, account) {
   ...
   *
   * @listens cipherguard.groups.update-local-storage
   * @param {uuid} requestId The request identifier
   */
  worker.port.on('cipherguard.groups.update-local-storage', async requestId => {
    try {
   ...
   * Find all the groups
   *
   * @listens cipherguard.groups.find-all
   * @param requestId {uuid} The request identifier
   * @param options {object} The options to apply to the find
   */
  worker.port.on('cipherguard.groups.find-all', async(requestId, options) => {
    try {
      const groupModel = new GroupModel(apiClientOptions);
   ...
   * Find all the groups
   *
   * @listens cipherguard.groups.find-my-groups
   * @param requestId {uuid} The request identifier
   * @param options {object} The options to apply to the find
   */
  worker.port.on('cipherguard.groups.find-my-groups', async requestId => {
   ...
   * Edit a groups
   *
   * @listens cipherguard.groups.update
   * @param requestId {uuid} The request identifier
   * @param groupDto {Object} The group object, example:
   *  {name: 'group name', groups_users: [{user_id: <UUID>, is_admin: <boolean>, deleted: <boolean>}]}
   */
  worker.port.on('cipherguard.groups.update', async(requestId, groupDto) => {
   ...
   */
  worker.port.on('cipherguard.groups.delete-dry-run', async(requestId, groupId, transferDto) => {
    try {
      const groupModel = new GroupModel(apiClientOptions);
   ...
   * example: {owners: [{aco_foreign_key: <UUID>, id: <UUID>}]}
   */
  worker.port.on('cipherguard.groups.delete', async(requestId, groupId, transferDto) => {
    try {
      const groupModel = new GroupModel(apiClientOptions);
      const transferEntity = transferDto ? new GroupDeleteTransferEntity(transferDto) : null;

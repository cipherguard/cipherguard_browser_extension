...
const listen = function(worker, apiClientOptions, account) {
   ...
   * @listens cipherguard.app.route-changed
   * @param path The relative navigated-to path
   */
  worker.port.on('cipherguard.app.route-changed', async path => {
    if (/^\/[A-Za-z0-9\-\/]*$/.test(path)) {
      const appBoostrapWorker = await WorkerService.get('AppBootstrap', worker.tab.id);
      appBoostrapWorker.port.emit('cipherguard.app-bootstrap.change-route', path);
    }
  });
  ...
  worker.port.on('cipherguard.account-recovery.get-organization-policy', async requestId => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
    const controller = new GetOrganizationPolicyController(worker, requestId, apiClientOptions);
    await controller._exec();
  });

  worker.port.on('cipherguard.account-recovery.save-organization-policy', async(requestId, accountRecoveryOrganizationPolicyDto, privateGpgKeyDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.validate-organization-key', async(requestId, newAccountRecoveryOrganizationPublicKey) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.generate-organization-key', async(requestId, generateGpgKeyDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.download-organization-generated-key', async(requestId, privateKey) => {
    try {
  ...
  worker.port.on('cipherguard.account-recovery.validate-organization-private-key', async(requestId, accountRecoveryOrganizationPrivateKeyDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.get-user-requests', async(requestId, userId) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.get-request', async(requestId, accountRecoveryRequestId) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.save-user-settings', async(requestId, accountRecoveryUserSettingDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.review-request', async(requestId, accountRecoveryRequestId, responseStatus, privateKeyDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.account-recovery.has-user-postponed-user-setting-invitation', async requestId => {
    const controller = new HasUserPostponedUserSettingInvitationController(worker, requestId);
    await controller._exec();
  });

  worker.port.on('cipherguard.account-recovery.postpone-user-setting-invitation', async requestId => {
    const controller = new PostponeUserSettingInvitationController(worker, requestId);
    await controller._exec();
  });

  worker.port.on('cipherguard.sso.get-current', async requestId => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
    const controller = new GetCurrentSsoSettingsController(worker, requestId, apiClientOptions);
    await controller._exec();
  });

  worker.port.on('cipherguard.sso.save-draft', async(requestId, draftSsoSettings) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.sso.dry-run', async(requestId, draftId) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.sso.activate-settings', async(requestId, draftId, ssoToken) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.sso.delete-settings', async(requestId, settingsId) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
    const controller = new DeleteSsoSettingsController(worker, requestId, apiClientOptions);
    await controller._exec(settingsId);
  });

  worker.port.on('cipherguard.sso.generate-sso-kit', async(requestId, provider) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.rbacs.find-me', async(requestId, name) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.password-policies.save', async(requestId, passwordSettingsDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.password-policies.get-admin-settings', async requestId => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.user-passphrase-policies.find', async requestId => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
    const controller = new FindUserPassphrasePoliciesController(worker, requestId, apiClientOptions);
    await controller._exec();
  });

  worker.port.on('cipherguard.user-passphrase-policies.save', async(requestId, userPassphrasePoliciesDto) => {
    const apiClientOptions = await User.getInstance().getApiClientOptions();
  ...
  worker.port.on('cipherguard.password-expiry.get-or-find', async(requestId, refreshCache = false) => {
  ...
  worker.port.on('cipherguard.password-expiry.save', async(requestId, passwordExpirySettingsDto) => {
  ...
  worker.port.on('cipherguard.password-expiry.delete', async(requestId, passwordExpiryId) => {
   ...
   *
   * @listens cipherguard.desktop.export-account
   * @param requestId {uuid} The request identifier
   */
  worker.port.on('cipherguard.desktop.export-account', async requestId => {
   ...
   *
   * @listens cipherguard.metadata.get-or-find-metadata-types-settings
   * @param requestId {uuid} The request identifier
   */
  worker.port.on('cipherguard.metadata.get-or-find-metadata-types-settings', async requestId => {

...
const listen = function(worker, apiClientOptions, account) {
   ...
   * Create a new folder
   *
   * @listens cipherguard.folders.create
   * @param requestId {uuid} The request identifier
   * @param folder {array} The folder
   */
  worker.port.on('cipherguard.folders.create', async(requestId, folderDto) => {
    try {
   ...
   * Update a folder
   *
   * @listens cipherguard.folders.update
   * @param requestId {uuid} The request identifier
   * @param folder {array} The folder
   */
  worker.port.on('cipherguard.folders.update', async(requestId, folderDto) => {
    try {
   ...
   * delete a folder
   *
   * @listens cipherguard.folders.delete
   * @param requestId {uuid} The request identifier
   * @param folder {array} The folder
   */
  worker.port.on('cipherguard.folders.delete', async(requestId, folderId, cascade) => {
    try {
      const folderModel = new FolderModel(apiClientOptions, account);
   ...
   *
   * @listens cipherguard.app.folders.update-local-storage
   * @param {uuid} requestId The request identifier
   */
  worker.port.on('cipherguard.folders.update-local-storage', async requestId => {
   ...
   *
   * @listens cipherguard.folders.find-details
   * @param requestId {uuid} The request identifier
   * @param folderId {uuid} The folder id
   */
  worker.port.on('cipherguard.folders.find-details', async(requestId, folderId) => {
   ...
   *
   * @listens cipherguard.folders.move-one-folder
   * @param {uuid} requestId The request identifier
   * @param {string} folderId The folder id to move
   * @param {string} destinationFolderId The destination folder id
   */
  worker.port.on('cipherguard.folders.move-by-id', async(requestId, folderId, destinationFolderId) => {

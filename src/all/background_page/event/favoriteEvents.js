...
const listen = function(worker, apiClientOptions, account) {
   ...
   *
   * @listens cipherguard.favorite.add
   * @param requestId {uuid} The request identifier
   * @param resourceId {uuid} The resource id
   */
  worker.port.on('cipherguard.favorite.add', async(requestId, resourceId) => {
    try {
   ...
   * @param resourceId {uuid} The resource id
   */
  worker.port.on('cipherguard.favorite.delete', async(requestId, resourceId) => {
    try {
      const favoriteModel = new FavoriteModel(apiClientOptions, account);

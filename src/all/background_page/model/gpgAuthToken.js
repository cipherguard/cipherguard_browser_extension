    ...
    switch (field) {
      case 'token' :
        ...
        if (sections[0] !== sections[3] && sections[0] !== 'gpgauthv1.3.0') {
          return new Error('Cipherguard does not support this GPGAuth version');
        }
        if (sections[1] !== '36') {
          return new Error(`Cipherguard does not support GPGAuth token nonce longer than 36 characters: ${sections[2]}`);
        }
        if (!Validator.isUUID(sections[2])) {
          return new Error('Cipherguard does not support GPGAuth token nonce that are not UUIDs');
        }
        return true;
      default :

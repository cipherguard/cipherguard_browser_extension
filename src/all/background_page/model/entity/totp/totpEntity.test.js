...
describe("Totp entity", () => {
  ...
  describe("::createTotpFromUrl", () => {
    ...
    it("should works if valid url is provided", () => {
      expect.assertions(1);
      const url = new URL('otpauth://totp/pro.cipherguard.local:admin@cipherguard.com?issuer=pro.cipherguard.local&secret=OFL3VF3OU4BZP45D4ZME6KTF654JRSSO4Q2EO6FJFGPKHRHYSVJA');
    ...
    it("should works if valid url with all possible parameters is provided", () => {
      expect.assertions(1);
      const url = new URL('otpauth://totp/pro.cipherguard.local:admin@cipherguard.com?issuer=pro.cipherguard.local&secret=OFL3VF3OU4BZP45D4ZME6KTF654JRSSO4Q2EO6FJFGPKHRHYSVJA&period=60&digits=8&algorithm=SHA256');
      const entity = TotpEntity.createTotpFromUrl(url);
      const dto = {
        secret_key: "OFL3VF3OU4BZP45D4ZME6KTF654JRSSO4Q2EO6FJFGPKHRHYSVJA",
        period: 60,
  ...
  describe("::createUrlFromExternalResource", () => {
    it("should return a valid url from TOTP", () => {
      expect.assertions(2);
      const urlExpected = new URL('otpauth://totp/pro.cipherguard.local%3Aadmin%40cipherguard.com?secret=DAV3DS4ERAAF5QGH&issuer=pro.cipherguard.local&algorithm=SHA1&digits=6&period=30');

      const entity = new TotpEntity(defaultTotpDto());
      const externalResourceDto = defaultExternalResourceDto({
        "name": "pro.cipherguard.local",
        "username": "admin@cipherguard.com",
        "uri": "pro.cipherguard.local",
      });

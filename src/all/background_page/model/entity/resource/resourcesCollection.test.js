...
import {defaultResourcesDtos, resourceAllTypesDtosCollection} from "cipherguard-styleguide/src/shared/models/entity/resource/resourcesCollection.test.data";
...
} from "cipherguard-styleguide/src/shared/models/entity/resourceMetadata/resourceMetadataEntity.test.data";
...
describe("ResourcesCollection", () => {
  ...
  describe("::constructor", () => {
    ...
    it("should, with enabling the ignore invalid option, ignore items which do not validate the unique id build rule", () => {
      const dto1 = defaultResourceDto({username: "user1@cipherguard.com"});
      const dto2 = defaultResourceDto({id: dto1.id, username: "user2@cipherguard.com"});
  ...
  describe("::filterBySuggestedResources", () => {
    it("should filter the collection by resources that could be suggested for a given url.", () => {
      const suggestedResource1 = defaultResourceDto({metadata: defaultResourceMetadataDto({uris: ["https://cipherguard.com"]})});
      const suggestedResource2 = defaultResourceDto({metadata: defaultResourceMetadataDto({uris: ["cipherguard.com"]})});
      const notSuggestedResource1 = defaultResourceDto({metadata: defaultResourceMetadataDto({uris: ["nost-cipherguard.com"]})});
      ...
      resources.filterBySuggestResources("https://www.cipherguard.com");
      expect.assertions(3);
    ...
    it("should filter all resources out if no resources could be suggested.", () => {
      const suggestedResource1 = defaultResourceDto({uri: "https://cipherguard.com"});
      const suggestedResource2 = defaultResourceDto({uri: "cipherguard.com"});
      const resources = new ResourcesCollection([
        suggestedResource1,
        suggestedResource2,
      ]);
      resources.filterBySuggestResources("https://www.not-cipherguard.com");
      expect.assertions(1);

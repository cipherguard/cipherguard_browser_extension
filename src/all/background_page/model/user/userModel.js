...
class UserModel {
   ...
   */
  /**
   * Create a user using Cipherguard API and add result to local storage
   *
  ...
  /**
   * Update a user using Cipherguard API and add result to local storage
   *
   * @param {UserEntity} userEntity
   * @param {boolean?} [ignoreInvalidEntity] Should invalid entities be ignored.
   * @returns {Promise<UserEntity>}
  ...
  async deleteDryRun(userId, transfer) {
    try {
      ...
      if (error instanceof CipherguardApiFetchError && error.data.code === 400 && error.data.body.errors) {
        /*

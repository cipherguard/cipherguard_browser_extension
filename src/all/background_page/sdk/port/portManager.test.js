...
describe("PortManager", () => {
  ...
  describe("PortManager::onPortConnect", () => {
    it("Should connect new port if it is in the workersSessionStorage", async() => {
      ...
      expect(port.postMessage).toHaveBeenCalledWith(JSON.stringify(['cipherguard.port.ready']));
    });

    it("Should connect new port if it is the quick access", async() => {
      expect.assertions(5);
      // data mocked
      const popupUrl = "chrome-extension://extensionId/webAccessibleResources/quickaccess.html?cipherguard=quickaccess";
    ...
    it("Should not connect new port if it is not the quick access url", async() => {
      expect.assertions(4);
      // data mocked
      const port = mockPort({name: "test", url: "chrome-extension://extensionId/webAccessibleResources/quickaccess.html?cipherguard=test"});
      delete port.sender.tab;
      // mock functions
      jest.spyOn(browser.action, "getPopup").mockImplementationOnce(() => "chrome-extension://extensionId/webAccessibleResources/quickaccess.html?cipherguard=quickaccess");
      jest.spyOn(PagemodManager, "attachEventToPort");
      // process
      try {
        await PortManager.onPortConnect(port);

...
(function() {
  class Stylesheet {
    ...
    handleStorageChange(changes) {
      if (changes._cipherguard_data && changes._cipherguard_data.newValue.config) {
        const config = changes._cipherguard_data.newValue.config;
    ...
    async getLocalStorage() {
      return new Promise(resolve => {
        chrome.storage.local.get(["_cipherguard_data"], result => resolve(result));
      });
    }
    ...
    async getThemeFromLocalStorage() {
      ...
      const {_cipherguard_data: {config}} = storageData;
    ...
    async isThemeDefined() {
      const storageData = await this.getLocalStorage();
      if (!storageData || !storageData._cipherguard_data) {
        return false;
      }
